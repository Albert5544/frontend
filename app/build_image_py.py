...
def clean_up_datasets(dataset_directory):
	...
	try:
		shutil.rmtree(os.path.join(instancePath, 'py_datasets', dataset_directory))
	except:
		try:
			os.remove(os.path.join(instancePath, 'py_datasets', dataset_directory))
		except:
			pass
...
def build_py_image(.....):
    ...
    if zip_file:  # if a set of scripts have been uploaded then its converted to a normal zip file format (ie. zip a folder)
        zip_path = os.path.join(instancePath,'py_datasets',zip_file) # instancePath -> key path in the server
        # unzip the zipped directory and keep the zip file
        with zipfile.ZipFile(zip_path) as zip_ref:
            dir_name = zip_ref.namelist()[0].strip('/')
			zip_ref.extractall(os.path.join(instancePath, 'py_datasets', dir_name))

        # find name of unzipped directory
        dataset_dir = os.path.join(instancePath, 'py_datasets', dir_name, dir_name)
        doi = dir_name
    else: 
		dataset_dir = os.path.join(instancePath, 'py_datasets', doi_to_directory(doi),doi_to_directory(doi))
		success = download_dataset(doi=doi, dataverse_key=dataverse_key,destination=os.path.join(instancePath, 'py_datasets',doi_to_directory(doi)))
	...
	user_defined_modules = files_list.generate_modules(dataset_dir) # set of all user defined modules
