...
# AST analyzer to gather infofrontend from Import and ImportFrom nodes
...
def get_imports(path,foldername):
    user_defined_modules = files_list.generate_modules(foldername) # set of all user defined modules
    import_sets = set()
    ...
    for i in import_sets:
        try:
            if(not(i in user_defined_modules) and not(i in built_in_modules2) and not(i in built_in_modules3)):
                subprocess.run(['conda', 'install','-y',i],check=True)
        except Exception:
            try:
                subprocess.run([os.path.expanduser("~")+'/anaconda3/envs/'+ foldername +'/bin/pip', 'install',i],check=True)
            except Exception:
                packages_to_ask_user.append(i)
                
    return packages_to_ask_user
