...
def path_preprocess(file_name,folder_name): 
    # file_name is absolute path of the file to be preprocessed
    root = os.getcwd() 
    hash = files_list.generate_multimap(folder_name)
    with open(file_name,'r') as infile:
        store = infile.readlines()
        dir_path = root+'/'+folder_name+'/'
        for j in range(len(store)):
            line = store[j]
            check = re.search(r"([\'\"]/)",line)  # Linux machine absolute paths
            ...
            check = re.search("[\'\"]\w:(.*)[\'\"]",line) # Windows absolute path type1
            ...
            check = re.search(r"([\'\"]\\)",line)  # windows absolute path type2
            ...
            if(check!=None):
                ...
                for i in arr:
                    ...
                    if(paths[1] == 'r' or paths[1]=='t' or paths[1]== 'n' or paths[1]== '' or paths[1]=='"' or paths[1]=='a' or paths[1]== 'b' or paths[1]=='f' or paths[1]=='N' ): # paths[0] will exists as \ is matched
                        continue
